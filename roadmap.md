...
| 項目 | ⏱ | ★ | 說明 |
|---|---|---|---|
| **`gomoku.session` 存了卻沒人讀** | 20 分 | ★★ | **白話**:每下一子,程式都把「這盤下到哪」寫進瀏覽器(棋盤大小、對戰模式、每一手棋),但**開機時從來沒有人把它讀回來**。所以關掉網頁再打開,棋盤是空的,那份存檔只是越寫越新、永遠沒人用。⇒ 兩個選項:①**做完它**——開機時問「要接續上一盤嗎?」重播棋譜(⚠ 解謎/每日模式刻意不存,見 CLAUDE.md 地雷 13);②**拆掉它**——沒人要接續就別留半套。★ 這是使用者 0903 問的,**等他拍板再動** |
